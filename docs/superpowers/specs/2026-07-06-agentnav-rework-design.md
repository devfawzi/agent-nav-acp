...
- **Rename bidirectionnel** : renommer un tab dans le plugin renomme la session côté Claude Code (visible ensuite dans le picker `/resume` du CLI). **✅ Mécanisme confirmé par sonde (2026-07-06, claude 2.1.201)** : control_request `{"subtype":"rename_session","title":"…"}` → `success`, titre persisté dans le `.jsonl` de la session. Le champ est `title` (pas `name`). Builder `ClaudeRequests.renameSession` + fixture `rename-session` en place — reste à câbler le rename de tab dessus (Plan 3). `/rename` et `set_session_title` n'existent pas en stream-json.
